...
np.random.seed(0)
'''
param = cPickle.load(open('item_kernel_2.pkl', 'rb'), encoding="latin1") 
print(param['V'])
param[1] = F.normalize(torch.FloatTensor(param['V']), p=2, dim=1)
print(param[1])

param1 = cPickle.load(open('../../../gan_2Ls_dpp/data/ml-100k-1/logistic_dpp_emb.pkl', 'rb'), encoding="latin1") 
print(param1['V'])
param1 = F.normalize(torch.FloatTensor(param1['V']), p=2, dim=1)
print(param1)
'''
...
#get processed sets
pos_sets, neg_sets = get_sets(path+'pos_item_sets.txt', path+'neg_item_sets.txt')
...
for run in range(runs):
    ...
    with tf.Session() as sess:
        ...
        cPickle.dump(param, open(path +'item_kernel.pkl', 'wb'))
